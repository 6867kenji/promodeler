...
def sample_measurements(rng: random.Random, population: dict) -> tuple[Measurements, dict]:
    ...
    shape = {"muscle": _round(muscle), "body_fat": _round(body_fat), "posture": _round(rng.gauss(0.5, 0.06), 3)}
    shape["posture"] = min(0.8, max(0.2, shape["posture"]))
    return measurements, shape


def sample_face(rng: random.Random, presets: dict) -> Face:
...
def sample_appearance(rng: random.Random, presets: dict, sex: str, race: str, catalog: Catalog, population: dict, warnings: list[RecipeWarning]) -> Appearance:
    ...
    facial_hair = None
    if sex == "male" and rng.random() < float(population.get("facial_hair_probability", 0.0)):
...
def sample_wardrobe(rng: random.Random, style: dict, sex: str, race: str, catalog: Catalog, warnings: list[RecipeWarning]) -> tuple[Garment, ...]:
    slots = style["wardrobe"].get(sex) or {}
...
def sample_accessories(rng: random.Random, style: dict, sex: str) -> tuple[Accessory, ...]:
    out = []
    for spec in style.get("accessories", []):
        ...
        if rng.random() >= float(spec.get("probability", 0.0)):
            continue
        prop = spec["id"].replace("-", "_")
        out.append(Accessory(id=spec["id"], source=AccessorySource(kind="promodeler_asset", path=f"assets/props/{prop}.py"), socket=spec["socket"]))
    return tuple(out)
...
def sample_character(seed: int, catalog: Catalog, presets: dict | None = None, sex: str | None = None, style: str | None = None,
                     recipe_id: str | None = None) -> tuple[CharacterRecipe, list[RecipeWarning]]:
    ...
    style_name = style or drawn_style
    ...
    measurements, shape = sample_measurements(rng, population)
    face = sample_face(rng, presets)
    appearance = sample_appearance(rng, presets, sex, race, catalog, population, warnings)
    wardrobe = sample_wardrobe(rng, styles[style_name], sex, race, catalog, warnings)
    accessories = sample_accessories(rng, styles[style_name], sex)
    recipe_id = recipe_id or f"random-{seed:08x}"
    recipe = CharacterRecipe(
        schema=SCHEMA, id=recipe_id, name=f"ランダム {seed:08x}（{'男性' if sex == 'male' else '女性'} {age} 歳、{style_name}）", seed=seed,
        source=Source(kind="random", path=str(Path(presets["_path"]).relative_to(PROJECT_ROOT)) if str(presets["_path"]).startswith(str(PROJECT_ROOT)) else presets["_path"],
                      sha256=presets["_sha256"], generator_version=SAMPLER_VERSION, catalog_version=catalog.version),
        identity=Identity(fictional=True, sex=sex, age=age, descriptors=(f"style:{style_name}",)),
        base=Base(race=race),
