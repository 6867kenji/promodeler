...
架空の日本人成人、30歳、裸足165cm。チャコールのパンツスーツ、アイボリーのブラウス、黒の低いパンプス、トープのトートバッグ。

ID `businesswoman` / category `female-character` / kind `humanoid` / 設計レビュー
...
```json
{
  ...
  "sex": "female",
  "age": 30,
  "face_shape_hair": "肩に触れるダークブラウンのボブ、卵型の顔",
  "outfit_palette": "チャコールのパンツスーツ、アイボリーのブラウス、黒の低いパンプス、トープのトートバッグ"
}
```

## 外観

肩に触れるダークブラウンのボブ、卵型の顔

## 身体寸法

```json
{
  "barefoot_height_m": 1.65,
...
```json
{
  ...
  "color_and_type": "チャコールのパンツスーツ、アイボリーのブラウス、黒の低いパンプス、トープのトートバッグ",
  "forefoot_sole_m": 0.008,
  "heel_height_m": 0.03,
  "fit_note": "踵30mm、前足部8mm。靴込み身長1.680mは踵加算の上限包絡で、姿勢と足裏角度による実測値を実装時に確定。"
}
```
...
|hair|#28201D|[0.25, 0.48]|0|肩に触れるダークブラウンのボブ、卵型の顔。束幅2–12mm、異方性、頭皮との境界を自然に。|
|top|#42464B|[0.65, 0.85]|0|細い綾織、襟の折返し、袖口の弱い摩耗。厚さと縫合はgarments表。|
|bottom|#42464B|[0.65, 0.85]|0|織目は脚の長手方向。股下の縫い合わせ、ポケット口、裾折返しを別形状で作る。|
|accent|#E5DFD1|[0.6, 0.85]|0|帯/エプロン/インナー/配色。異なる生地は必要に応じてスロット分離。|
...
```json
{
  ...
  "description": "架空の日本人成人、30歳、裸足165cm。チャコールのパンツスーツ、アイボリーのブラウス、黒の低いパンプス、トープのトートバッグ。"
}
```
