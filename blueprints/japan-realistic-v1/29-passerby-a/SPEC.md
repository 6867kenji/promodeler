...
架空の日本人成人、20歳、裸足180cm。ライトグレーのパーカ、黒カーゴパンツ、グレーのスニーカー、ネイビーのリュック。

ID `passerby-a` / category `male-character` / kind `humanoid` / 設計レビュー
...
```json
{
  ...
  "sex": "male",
  "age": 20,
  "face_shape_hair": "黒の短いマッシュ髪、細長い顔",
  "outfit_palette": "ライトグレーのパーカ、黒カーゴパンツ、グレーのスニーカー、ネイビーのリュック"
}
```
...
```json
{
  "barefoot_height_m": 1.8,
  "standing_shod_height_m": 1.83,
  "shoe_sole_m": 0.03,
  "age": 20,
  "shoulder_width_m": 0.43,
  "inseam_m": 0.84,
  "foot_length_m": 0.27,
...
[
  {
    ...
    "finished_measurements_m": {
      "back_length": 0.72,
      "chest_circumference": 1.1400000000000001,
      "hood_height": 0.34,
      "cord_diameter": 0.004
    },
    "construction": "二枚フード、カンガルーポケット、袖口リブ幅65mm。フードは背中に下ろす。",
    "deformation": "cloth"
  },
  {
    "id": "lower",
...
```json
{
  ...
  "color_and_type": "ライトグレーのパーカ、黒カーゴパンツ、グレーのスニーカー、ネイビーのリュック",
...
|hair|#28201D|[0.25, 0.48]|0|黒の短いマッシュ髪、細長い顔。束幅2–12mm、異方性、頭皮との境界を自然に。|
|top|#B8B9B7|[0.65, 0.85]|0|裏毛のループ、フード紐4mm、リュックの織りとファスナー。厚さと縫合はgarments表。|
|bottom|#303336|[0.65, 0.85]|0|織目は脚の長手方向。股下の縫い合わせ、ポケット口、裾折返しを別形状で作る。|
|accent|#253952|[0.6, 0.85]|0|帯/エプロン/インナー/配色。異なる生地は必要に応じてスロット分離。|
...
```json
{
  ...
  "description": "架空の日本人成人、20歳、裸足180cm。ライトグレーのパーカ、黒カーゴパンツ、グレーのスニーカー、ネイビーのリュック。"
}
```
