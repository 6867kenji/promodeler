...
架空の日本人成人、34歳、裸足168cm。サンドベージュの半袖ブラウス、テラコッタの足首丈スカート、茶色フラットシューズ、茶色ショルダーバッグ。

ID `passerby-b` / category `female-character` / kind `humanoid` / 設計レビュー
...
```json
{
  ...
  "sex": "female",
  "age": 34,
  "face_shape_hair": "ダークブラウンのゆるい肩下ウェーブ、長めの卵型顔",
  "outfit_palette": "サンドベージュの半袖ブラウス、テラコッタの足首丈スカート、茶色フラットシューズ、茶色ショルダーバッグ"
}
```

## 外観

ダークブラウンのゆるい肩下ウェーブ、長めの卵型顔

## 身体寸法

```json
{
  "barefoot_height_m": 1.68,
  "standing_shod_height_m": 1.695,
  "shoe_sole_m": 0.015,
  "age": 34,
...
[
  ...
  },
  {
    ...
    "material": "bottom",
    "finished_measurements_m": {
      "waist_circumference": 0.735,
      "length": 0.88,
      "hem_circumference": 1.65,
      "thickness": 0.0007
    },
    "construction": "6枚接ぎ、左脇コンシールファスナー。腰から縦ドレープ。",
    "deformation": "cloth"
  }
]
```
...
```json
{
  ...
  "color_and_type": "サンドベージュの半袖ブラウス、テラコッタの足首丈スカート、茶色フラットシューズ、茶色ショルダーバッグ",
  "forefoot_sole_m": 0.015,
  "heel_height_m": 0.015
}
```
...
|hair|#28201D|[0.25, 0.48]|0|ダークブラウンのゆるい肩下ウェーブ、長めの卵型顔。束幅2–12mm、異方性、頭皮との境界を自然に。|
|top|#D6C4A6|[0.65, 0.85]|0|ブラウスの細い織り、スカートの縦ドレープ、革のしぼ。厚さと縫合はgarments表。|
|bottom|#A95F48|[0.65, 0.85]|0|織目は脚の長手方向。股下の縫い合わせ、ポケット口、裾折返しを別形状で作る。|
|accent|#604737|[0.6, 0.85]|0|帯/エプロン/インナー/配色。異なる生地は必要に応じてスロット分離。|
...
```json
{
  ...
  "clothing": {
    ...
    "range": "スカート裾は静止形状から180mm以内。着座では布を座面・大腿で支持し、静止裾位置からの距離だけで拘束しない。"
  },
  "breast": {
...
```json
{
  ...
  "description": "架空の日本人成人、34歳、裸足168cm。サンドベージュの半袖ブラウス、テラコッタの足首丈スカート、茶色フラットシューズ、茶色ショルダーバッグ。"
}
```
