...
- Cloth garments for the character creator are `assets/wardrobe/<name>.py` generators cut on a race profile (`character/profiles/<race>.json` from `promodeler character profile <race>`): `Loft` sections through the profile's torso outlines and arm cross-sections plus ease, in the Unity body frame (metres, Y up, faces +Z, UMA 3 rests in an A-pose). A garment cut for one body moves to another with `promodeler character fit-garment` (offsets from the body surface are kept; an external body needs `character profile-from-mesh` with its joint positions). They become UMA content with `promodeler character import-slot --manifest character/garments.json`; list each garment in that manifest and point the catalog entry's `runtime.uma_wardrobe_recipe_by_race` at `<name>_Wardrobe`.
- Do not build new hair, dresses or shoes from `Strands` / `Loft` as standalone humanoid parts; those are catalog assets. Props a character carries (bags, glasses, watches) stay ordinary `assets/props/<name>.py` assets referenced from `accessories[].source`: constant materials (no bakes), a `size` parameter the recipe overrides, `promodeler.props` helpers, closed rings as `Revolve` tori (a closed `Sweep` self-intersects at its caps), and `Asset(extras=props.socket_extras(socket, grip_offset_m, orientation))` so the Unity side knows where the object is held. A garment that is rigid enough (necktie, belt, badge) can also be a prop: give the catalog entry `runtime.promodeler_asset`, `runtime.socket` (`neck`, `waist`, `chest`) and `runtime.parameters` (prop parameter <- `garment.<finished_measurement>` / `garment.material.<field>` / `body.<measurement>` expressions), and the character build attaches it instead of a UMA recipe (`assets/props/necktie.py`, `assets/props/obi_belt.py`).
...
- Check the fitted measurements in `build/human/<name>-<hash>/rig.json` (`measurements`) against the blueprint before judging renders; a few millimeters on lengths and 1 to 2 cm on circumferences is the current accuracy. Circumferences alone are not enough: MHR reaches a large bust by widening the chest, so `shoulder_width` is the outer slice width at the shoulder landmark (the joint distance ends up about 7 cm smaller), bust width and depth are fitted, and a geometric bust amplitude (`bust_field`) supplies the depth the identity space lacks. If a body reads as broad or flat, look at `bust_depth`, `shoulders_width` and `bust_bump_m` in the measurements before touching the asset.
...
- Face shapes: `fitted_body` exports MHR expression deltas (`FACE_SHAPES`: blink_l/r, jaw_open, smile, mouth_wide, pucker, vowel_a..o) as shape keys; drive them with `Pose(id, {}, shapes={"blink_l": 1.0})`, render with `--pose blink`, and key them in clips like any pose (idle blink at 1.1 s, a `speak` vowel sequence). Check `report.shape_keys` and the glb's `targetNames`; a pose naming an unknown shape fails with `pose.shape`.
