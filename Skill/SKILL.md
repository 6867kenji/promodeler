...
- Cloth garments for the character creator are `assets/wardrobe/<name>.py` generators cut on a race profile (`character/profiles/<race>.json` from `promodeler character profile <race>`): `Loft` sections through the profile's torso outlines and arm cross-sections plus ease, in the Unity body frame (metres, Y up, faces +Z, UMA 3 rests in an A-pose). They become UMA content with `promodeler character import-slot --manifest character/garments.json`; list each garment in that manifest and point the catalog entry's `runtime.uma_wardrobe_recipe_by_race` at `<name>_Wardrobe`.
